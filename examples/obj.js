...
/* global ModelViewer, handlers, quat, vec3 */
...
const texture = viewer.load( "texture.bmp", pathSolver );
...
const model = viewer.load( "cube.obj", pathSolver );
...
const instance2 = model.addInstance().move( [ - 150, 0, 0 ] ).scale( [ 0.5, 2, 5 ] );
...
const q = quat.setAxisAngle( [], vec3.normalize( [], [ 1, 1, 1 ] ), Math.PI / 120 );
const q2 = quat.setAxisAngle( [], vec3.normalize( [], [ 1, 1, 1 ] ), Math.PI / 30 );
